...
const ShippingForm = ({setShippingForm}:{setShippingForm:(data:ShippingFormInputs)=>void}) => {
  ...
  return (
    <form action="" className='flex flex-col gap-4' onSubmit={handleSubmit(handleShippingForm)}>
        ...
        <div className='flex flex-col gap-1'>
            ...
            <input className="border-b border-gray-200 py-2 outline-none text-sm" type="text" id="phone" placeholder='1234567891' {...register("phone")}/>
            {errors.phone && <p className='text-xs text-red-500'>{errors.phone.message}</p>}
        </div>
        <div className='flex flex-col gap-1'>
            <label htmlFor="address" className='text-xs text-gray-500 font-medium'>Address</label>
            <input className="border-b border-gray-200 py-2 outline-none text-sm" type="text" id="address" placeholder='123 Main st' {...register("address")}/>
